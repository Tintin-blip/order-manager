...
export const getOrderWithProductDetails = async (req = request, res = response) => {
    try {
        const { idUser } = req.params;
        const { quantity, idProduct } = req.body;
        ...
        const sql1 = "INSERT INTO orders(id_user) VALUES (?)";
        const values1 = [idUser];
        const query1 = await pool.query(sql1, values1);


        const sql2 = "SELECT id, name, price FROM products WHERE id = ?";
        const values2 = [idProduct];
        const product = await pool.query(sql2, values2);
        const priceProduct = product[0][0].price;


        const orderId = query1[0].insertId;
        const sql3 = "INSERT INTO orders_details(id_order, id_product, price, quantity) VALUES (?, ?, ?, ?)";
        const values3 = [orderId, idProduct, priceProduct, quantity];
        const insertToOrder = await pool.query(sql3, values3);


        const orderData = {
            orderId,
            idProduct: product[0][0].id,
            productName: product[0][0].name,
            productPrice: product[0][0].price,
            quantity,
